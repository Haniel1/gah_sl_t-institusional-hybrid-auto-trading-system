...
import { ArrowLeft, Shield, Bell, CheckCircle2, Loader2, UserPlus, Users, Trash2, LogOut, KeyRound } from 'lucide-react';
...
export default function Settings() {
  ...
  const [newUser, setNewUser] = useState({ name: '', username: '', password: '', indodax_api_key: '', indodax_secret: '', telegram_bot_token: '', telegram_chat_id: '' });
  ...
  const addUser = async () => {
    ...
    try {
      ...
      if (!res.ok || !data.success) {
        ...
        setNewUser({ name: '', username: '', password: '', indodax_api_key: '', indodax_secret: '', telegram_bot_token: '', telegram_chat_id: '' });
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="max-w-2xl mx-auto p-6">
        ...
        {/* Security Notice */}
        <div className="terminal-card p-4 mb-6 border-terminal-yellow/30">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                API Key Indodax dan Telegram Token tersimpan sebagai Cloud Secrets (terenkripsi). 
        ...
        {/* User Management */}
        <section className="terminal-card p-4 mb-4">
          ...
          {/* Add User Form */}
          {showAddUser && (
            <div className="border border-primary/20 rounded-md p-4 space-y-3 bg-primary/5">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-2.5 py-1.5 rounded-md border border-border bg-background text-xs text-foreground focus:outline-none focus:ring-1 focus:ring-primary/50"
              ...
              <div className="border-t border-border pt-3 mt-2">
                <p className="text-[10px] text-muted-foreground mb-2 font-semibold">Kredensial Trading (opsional, bisa diisi nanti)</p>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-[10px] text-muted-foreground mb-1 block">Indodax API Key</label>
                    <input
                      type="text"
                      value={newUser.indodax_api_key}
                      onChange={e => setNewUser(p => ({ ...p, indodax_api_key: e.target.value }))}
                      className="w-full px-2.5 py-1.5 rounded-md border border-border bg-background text-xs text-foreground font-mono focus:outline-none focus:ring-1 focus:ring-primary/50"
                      placeholder="API Key"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      type="password"
                      value={newUser.indodax_secret}
                      onChange={e => setNewUser(p => ({ ...p, indodax_secret: e.target.value }))}
                      className="w-full px-2.5 py-1.5 rounded-md border border-border bg-background text-xs text-foreground font-mono focus:outline-none focus:ring-1 focus:ring-primary/50"
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      value={newUser.telegram_bot_token}
                      onChange={e => setNewUser(p => ({ ...p, telegram_bot_token: e.target.value }))}
                      className="w-full px-2.5 py-1.5 rounded-md border border-border bg-background text-xs text-foreground font-mono focus:outline-none focus:ring-1 focus:ring-primary/50"
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={e => setNewUser(p => ({ ...p, telegram_chat_id: e.target.value }))}
                      className="w-full px-2.5 py-1.5 rounded-md border border-border bg-background text-xs text-foreground font-mono focus:outline-none focus:ring-1 focus:ring-primary/50"
                      placeholder="Chat ID"
                    />
                  </div>
        ...
        <section className="terminal-card p-4 mb-4">
          ...
          {showChangePassword && (
            <div className="space-y-3">
              {pwError && (
                <div className="text-xs text-loss bg-loss/10 border border-loss/20 rounded-md p-2">{pwError}</div>
              )}
              {pwSuccess && (
